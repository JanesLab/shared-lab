function sol=NRF2_p53_ODE_DCIS(invar,shNRF2,p53DD,MaxTime)
% NRF2-p53 base model (MCF10DCIS.com)

%First order reactions (min^-1), Second order (Cs-1*min^-1), unless
%otherwise noted 

k_keap1ox = 0.0084; %Oxidation rate of KEAP1 - https://www.ncbi.nlm.nih.gov/pubmed/24634836, https://www.ncbi.nlm.nih.gov/pubmed/20061377,
                    % already multiplied by 7 to account for extra/intracellular gradient of H2O2
...
k_nrf2deg = 0.02; %Nrf2 degradation rate (experimentally - see NRF2 degradation blot), 0.02 
...
k_pATMred=1/60; %basal rate of dephosphorylation (other phosphatases besides Wip1 acting on pATM)
k_syn_ATM=0.003; %Synthesis rate of ATM 
k_deg_ATM=0.0005; %Degradation rate of ATM 

basal_syn_R=0.0006; %Basal R synthesis rate
                    
%Parameters adjusted for DCIS model:
k_prod_H2O2=.0052; %Basal H2O2 generation rate, fold-change increase over parameter in 5E model determined based on ~1.3 fold increase in Hyper ratio in DCIS.com cells compared to 5E cells (Fig. s7D)     
k_deg_R=0.01; %turnover rate of R reacting with other oxidized proteins in cell not taken into account in this model, increased from 5E to handle increase in H2O2 production rate

% Initial conditions adjusted to reflect proportional differences in species 
%abundance estimated from RNA-seq (in 'SpeciesICs_DCIS*.csv' files)
y0 = zeros(22,1); 

if isfile('SSvalues_DCISbase.mat')
    load('SSvalues_DCISbase.mat','SS')
    y0=SS;
else
    vals=readtable('SpeciesICs_DCIS.csv','ReadRowNames',true);
    y0=[
...
if shNRF2==1 && p53DD ==1
    ...
    beta_R_p53=0; %R30
    if isfile('SSvalues_DCISshNRF2p53DD.mat')
        load('SSvalues_DCISshNRF2p53DD.mat','SS')
        y0=SS;
    else
        vals=readtable('SpeciesICs_DCISshNRF2.csv','ReadRowNames',true);
        y0=[
            1.74985959173128 * vals{'KEAP1','FC'}
            0.000127644027974550 * vals{'KEAP1','FC'}
            0.0717899022306803 * vals{'NFE2L2','FC'}
...
elseif shNRF2==1 && p53DD ==0
    Ksyn_nrf2=0.005/5;
    if isfile('SSvalues_DCISshNRF2.mat')
        load('SSvalues_DCISshNRF2.mat','SS')
        y0=SS;
    else
        vals=readtable('SpeciesICs_DCISshNRF2.csv','ReadRowNames',true);
        y0=[
...
elseif p53DD==1 && shNRF2 ==0
    ...
    alpha_mrna_p21=0; %R28
    beta_R_p53=0; %R30
    load('SSvalues_DCISp53DD.mat','SS')
    y0=SS;
end
...
%     alpha_mrna_p21=0; %R28
%     beta_R_p53=0; %R30
%     load('SSvalues_DCISshNRF2p53DD_noshNRF2ICs.mat','SS')
%     y0=SS;
% elseif shNRF2==1 && p53DD ==0
%     Ksyn_nrf2=0.005/5;
%     load('SSvalues_DCISshNRF2_noshNRF2ICs.mat','SS')
%     y0=SS;
% elseif p53DD==1 && shNRF2 ==0
%     beta_M=0; %R19
%     beta_i=0; %R23
...
ATM=sol.y(18,:); %ATM
p21_mrna=sol.y(19,:); %p21_mrna
pATM=sol.y(20,:); %pATM
p21_prot=sol.y(21,:);%p21_prot
